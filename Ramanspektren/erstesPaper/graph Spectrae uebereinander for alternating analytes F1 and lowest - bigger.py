...
#    print(z)
 ...
 #        print(isinstance(y_values[y_values.columns[m]][56], str))
  ...
  #  print(nrCol)
    ...
    verticalline1 = 1213
    verticalline2 = 1391
    verticallinelength = [0, shift[0] + max(y_values.iloc[:,-1])-2000]
...
#    plotly.offline.plot(fig, filename=nwfile) #, auto_open=False) #,  image_filename=nwfile)  #, image='png', image_width=1600, image_height=860)
    plotly.offline.plot(fig, filename=nwfile, auto_open=True, image_filename=nwfile, image='svg', image_width=600, image_height=500)



for dateiname in os.listdir():
    if dateiname.endswith('4.csv'):
        print(dateiname)
